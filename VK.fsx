...
let searchFeed st et =
    let startTs = toUnixTime st |> int64
    let endTs = toUnixTime et |> int64
    let data = 
        Http.RequestString(
            sprintf "https://api.vk.com/method/newsfeed.search?v=5.50&extended=1&count=200&start_time=%i&end_time=%i" startTs endTs
        )
    JsonConvert.DeserializeObject<NewsfeedVkResponse<VkCollection<NewsfeedEntry>>>(data)
